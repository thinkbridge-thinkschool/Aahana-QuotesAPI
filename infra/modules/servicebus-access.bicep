...
// servicebus.bicep's namespace/queue outputs to even start (they're baked into its container
// env vars). Two modules needing each other's outputs is a cycle Bicep's dependency graph can't
// resolve — splitting the RBAC grant out into its own module, run after both, avoids it: main.bicep
// deploys sql + serviceBus -> api (using their outputs) -> this module (using api's output).
resource serviceBusNamespace 'Microsoft.ServiceBus/namespaces@2023-01-01-preview' existing = {
  name: namespaceName
}

// Azure Service Bus Data Owner — send + receive + manage. A production hardening pass would
// split this into narrower Data Sender/Data Receiver assignments per consumer; one Owner grant
// is the pragmatic kickoff-scope choice, called out rather than silently left broad.
