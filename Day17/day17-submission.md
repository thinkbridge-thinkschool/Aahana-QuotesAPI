...
- **Error state (UI)**: not visually verified in a browser this session — the app's `error.interceptor.ts` is unchanged from Day 15 and structurally handles non-2xx responses, but I did not drive a browser against the live site to confirm the rendered error UI. Flagging this honestly rather than claiming a check that didn't happen.

## Three concrete bugs I caught and fixed during the live deployment
